...
module.exports = {
  ...
  elevate: function(cmd,options,callback) {
    ...
    // get the right elevate.cmd path in an electron app
    var electronPath = path.join(
        process.argv[6].replace("--app-path=", ""),
        "node_modules", "node-windows", "bin", "elevate", "elevate.cmd"
    );
    // exec with that right path
    exec('"'+electronPath+'" '+cmd,p.options,p.callback);
  },
